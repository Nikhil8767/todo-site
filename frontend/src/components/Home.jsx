...
const Home = () => {
  ...
  const addOrUpdateTask = () => {
    ...
    if (editIndex !== null) {
      ...
      updatedTasks[editIndex] = { work, time, completed: false }; // Ensure 'completed' is set when updating
      setTask(updatedTasks);
      setEditIndex(null); // Reset edit index
    } else {
      // Add new task
      const newTask = { work, time, completed: false }; // Add 'completed' key to new tasks
            ...
            {task.map((t, index) => {
              return (
                <li className='flex justify-between items-center border-b border-gray-300 py-2' key={index}>
                  <input type="checkbox" className='mr-4' checked={t.completed} onChange={() => toggleCompletion(index)} />
                  <span className={`text-lg font-semibold text-gray-800 ${t.completed ? 'line-through text-gray-500' : ''}`}>
                    {t.work} at {t.time}
                  </span>
